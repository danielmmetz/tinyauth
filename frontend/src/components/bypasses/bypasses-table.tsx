...
export const BypassesTable: React.FC = () => {
  ...
  const { data: domainsData } = useQuery({
    ...
    queryFn: () =>
      axios
        .get("/api/bypasses/domains")
        .then((res) => (res.data.domains || []) as string[]),
  });

  const displayCidr = !isAdmin ? clientIP : formData.cidr;

  const deleteMutation = useMutation({
    mutationFn: async (id: number) => axios.delete(`/api/bypasses/${id}`),
    onSuccess: () => {
    ...
    <div className="overflow-x-auto border rounded-lg">
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          {/* Inline form row for creating a new bypass */}
